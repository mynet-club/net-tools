...
| POST | `/v1/_admin/providers/{name}/test` | **快速测试**：直接打这家系统上游（body 可给 `{"model":…}`，不给则用它声明的第一个） |
...
**3. 快速测试 —— 每条映射、每家上游旁边都有「测试」。**
按下去就真发一条 `max_tokens=8` 的极小请求，回报**哪家上游接的、真正打出去的上游模型名、
耗时、以及上游回的那句话**。模型名写错、密钥过期、地址配错，当场就能看出来。
...
用户详情里的「模型范围」也按这个思路做：**继承模式下会把继承到的模型逐个列出来，各带一个「测试」**。
继承是默认路径（见下一节），不列出来的话，快速测试对绝大多数用户根本够不着 ——
而且「继承」不等于「上游一定可用」，逐个测一遍才算真的确认过。
