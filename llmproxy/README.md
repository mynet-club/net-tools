...
| 请求情况 | 网关怎么做 |
|---|---|
| 带会话头，且那家在候选池里、健康 | 一直用它（观测头 `sticky`） |
| 那家挂了 / 返回 4xx（这家用不了） | 本次漂到别家（观测头 `drift`），并**把粘性更新成新家** |
...
**观测**：响应头 `X-Llmproxy-Affinity` 三个值 —— `new`（还没记录）/ `sticky`（按粘性选中）/
`drift`（漂移过来的）。排障与上线验证都看这一列。
