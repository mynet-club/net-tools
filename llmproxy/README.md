...
> 📖 **第一次接触这个项目？** 先读 [docs/OVERVIEW.md](./docs/OVERVIEW.md) ——
> 那里讲「它是什么、为什么这么设计、哪些地方故意不做」，带架构图与实测数据。
> 本文件是**操作手册**：完整配置项、每个 HTTP 接口、CLI 全部子命令、开发指南。
...
**上游错误响应体会被留下来，但只给运营者看。** 上游返回可重试的错误状态码时，
响应体前 300 字节会写进日志、并存进 `provider_stats.last_error`（排障要用它 ——
不知道上游为什么 4xx 就没法查）。这与上面「不记录请求/响应正文」是冲突的，
所以把**下发面**收窄了：`GET /v1/_providers` 只对静态 api_key（也就是
`llmproxy providers` 用的那个）与 BYO 用户开放，**消费模式用户一律 403** ——
他们没有自己的上游，调这个接口只会看到系统池的 `base_url`、内联代理 URL
（常带 `user:pass`）与系统供应商的错误正文，而用户台压根不调它
（它用的是 `/v1/_me` 的 `broken_providers`）。BYO 用户看到的是**自己**那些上游的
状态，那本来就是他们自己的东西。
...
| 那家挂了 / 返回 4xx / 流中途失败（这家用不了） | 本次漂到别家，并**把粘性更新成新家** | `drift` |
| 没有粘性可用（新会话，**或压根没带会话头**），且有价目可挑 | **按当前上游价挑最便宜的**（规则 B，见下） | `cheapest` |
| 没有价目可挑（或价目币种与基准币不符） | 照旧按权重随机，不碰粘性表 | `new` |
| 全部候选都失败 | 忘掉粘性，下一个请求重新选 | — |

**观测头只在带了会话头时才写**（`affinity_ttl_ms` 显式设 0 关掉粘性时也不写）。
所以没带会话头的请求即使命中了规则 B，也看不到 `cheapest` —— 选路确实按价格走了，
只是没有一个会话可以粘。想观测规则 B 就得让下游发 `x-session-affinity`。
...
**两种模式不是互斥的。** 消费模式下的用户**照样可以配自己的上游**，而且它们**优先**：

```
自有上游（承接这个模型）  →  系统池
    ↑ 用户自己结算，不进配额        ↑ 网关付费，计入配额与金额
```

意思就是「自己有两个 token plan（A / B），A 花完切 B，两个都没额度了才动网关的系统池」。
选路顺序是：自有·点名·健康 → 自有·点名 → 自有·兜底 → 系统·点名 → 系统·兜底（详见
`internal/router/router.go` 里挑池子那一段）。**配额只卡系统那一段**：额度用完时，
自有上游承接的模型照旧放行。
...
1. **两种模式可以叠加，自有上游优先**。`byo` 只走用户自己的上游；`consumption` 在系统池之外
   **也认用户自己配的上游**——命中自有上游就走它（用户自己和供应商结算，不进配额），
   没命中、或自有全部不可用时才回落到系统池（那部分才计网关的账与配额）。
   所以「自己有 A/B 两个 token plan，A 花完自动切 B；B 也没额度了才动网关的系统池」是可得的行为。
2. **配额是软限制**。判定发生在请求之前，并发请求最多可能超出「同时在飞」的那几条；
   跨过上限的那一条会放行，之后的才拒。要精确到分就不是估算，那是支付系统的活。
   配额只约束**系统付费**的部分：额度用完时，自有上游承接的模型照旧放行。
