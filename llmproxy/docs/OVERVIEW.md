...
它只有 `admin_token` 能调，而 admin 本来就能读 config.yaml、读所有用户数据、
改生产配置，**不构成权限跨越**（审阅自己也把它标为「中等偏上」，放进严重段只是因为
它属于 SSRF 这一类）。**中等里凡是影响钱、可用性或安全的都修**；轻微按需。
共改动 19 个生产文件，每一项都配回归测试，关键几条都**临时回退验证过测试确实会失败**。

**未修的中等里，值得知道的几个：**

- DeepSeek 形状与 `cache_write_tokens` 并存时会重复计费 —— 代码路径确认可达，
  但无法验证是否真有上游这么回报，所以没动
- `Prune` 会删掉冻结账本 —— 不是 bug，是「请求明细保留天数」与「账本可审计」的
  设计冲突，已写进 [pricing-design.md](./pricing-design.md) 的「实现备注」
- 价目表没有 UNIQUE 约束 —— 进程内被单连接串行化保护，跨进程能否触发未验证
- `InsertRequest` 失败只打一行日志 —— 账会静默丢，没有重试也没有计数器
