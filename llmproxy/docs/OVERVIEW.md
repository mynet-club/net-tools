...
四路共报告 **9 个严重级 + 29 个中等 + 26 个轻微**（另有若干观察项，且跨路有重复 ——
比如「计费可被绕过」被转发与计费两路各自独立发现）。

处理原则：**严重级全修**（唯一例外是「管理台 `/test` 回显上游响应体」——
它只有 admin_token 能调，而 admin 本来就能读 config.yaml 与所有用户数据，
不构成权限跨越）；**中等里凡是影响钱、可用性或安全的都修**；轻微按需。
...
| 6 | 客户端断开被赖到供应商头上 | **一次断开就足以把整个供应商池打进冷却** |
| 7 | `rowCharge` 摊分分母含失败请求 | 按失败率线性多收（10% 失败率 → 多收 10%），且进程重启会凭空抬高用户当月已用金额 |
| 8 | `in_write` 缺省不回落 | 缓存写入档 token 完全不计费（该档单价通常比未命中还贵） |
| 9 | 流中途失败不进熔断 | 「返回 200 头然后卡死」的供应商永久健康，粘性还会把会话钉死在它上面 |
| 10 | 非流式响应无上限缓冲 | 一个 BYO 用户一次请求就能把网关内存吃光；且中途失败会发出 200 + 截断 body |
| 11 | `retain_days: 0` 与文档相反 | 照文档配「永久保留」的人会在第 90 天丢掉计价冻结账本 |
| 12 | 币种无任何护栏 | 规则 B 可能选出贵 3~7 倍的供应商 |
| 13 | `yamlScalar` 手写转义漏了 lone CR | 带 CR 的供应商名能逃出 `providers` 段注入新的顶层段（有可落盘 PoC） |
| 14 | PRAGMA 不在 DSN + deferred 事务跨进程 BUSY | 连接重建后静默降级；CLI 与服务端并发写随机报 `database is locked` |
| 15 | `DeleteUser` 不级联 | 删用户再建同名会**静默继承前任的模型授权**与熔断计数 |
| 16 | `Stats(since)` 只过滤总计 | `stats --days 1` 打印「总计: 请求 1」，紧接着的按供应商表却是全量历史 |
| 17 | 粘性表满后 O(n²) 淘汰 | 3500 倍性能悬崖，且压测工具不发粘性头，完全看不见 |
