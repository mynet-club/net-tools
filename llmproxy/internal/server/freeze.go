...
// 算法在 store.RowCharge —— CLI、API、服务端共用一份，避免口径漂移。
func rowCharge(r store.UsageRow, p *config.PricingConfig, now time.Time) float64 {
	return store.RowCharge(r, pricingCost(p), now)
}

// pricingCost 把 PricingConfig 适配成 store.CostFunc。
func pricingCost(p *config.PricingConfig) store.CostFunc {
